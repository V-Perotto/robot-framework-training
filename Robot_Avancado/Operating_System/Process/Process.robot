...
Verifica sucesso na execução do processo
    ...
    Run Keyword If   ${PROCESSO.rc}!=0   Log   FALHA OCORRIDA: "${PROCESSO.stderr}"     # Descrição do erro que ocasionou

Execute um script e aguarde ele finalizar com sucesso
    ## Inicia o processo e espera pelo seu término
    # Run Process normalmente é utilizado para arquivos
    ${MEU_PROCESSO}     Run Process     python   HelloWorld.py
    Log    Resultado da execução (sucesso/falha): ${MEU_PROCESSO.rc}    # rc é o retorno --> 0 (sucesso) ou 1 (falha) 
    Log    Saída da execução: ${MEU_PROCESSO.stdout}                    # stdout é a saída que o script retornou
    Verifica sucesso na execução do processo      ${MEU_PROCESSO}

Execute um script e aguarde ele finalizar com falha
    ## Inicia o processo e espera pelo seu término
    ${MEU_PROCESSO}     Run Process     python   HelloWorld_Fail.py
